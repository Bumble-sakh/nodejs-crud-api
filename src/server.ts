...
const createRequestListener = (port: number) => {
  return (req: IncomingMessage, res: ServerResponse) => {
    const [prefix, endpoint, id] = req.url.split("/").slice(1);
    const method = req.method;
    const endpointPath = req.url || "/";

    // Логирование запроса для проверки балансировщика
    console.log(
      `Worker ${process.pid} on port ${port} received ${method} ${endpointPath}`
    );
...
export const runServer = (port: number) => {
  const requestListener = createRequestListener(port);
